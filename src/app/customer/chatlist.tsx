...
export default function CustomerChatList() {
  ...
  useEffect(() => {
    console.log(`🔍 CUSTOMER CHATLIST - Current user from store:`, currentUser);
    console.log(`🔍 CUSTOMER CHATLIST - Current user ID:`, currentUser?._id);
    console.log(`🔍 CUSTOMER CHATLIST - Current user name:`, currentUser?.firstName, currentUser?.lastName);
  }, [currentUser]);
    ...
    if (otherParticipant) {
      router.push({
        ...
        params: {
          ...
          vehicleType: otherParticipant.userId.vehicleType || "",
        },
      });
    }
  };
  ...
  const handleOnlineUserPress = (user: OnlineUser) => {
    ...
    const handleChatCreated = (data: any) => {
      console.log("✅ Chat created via socket:", data.chat._id);
    ...
    const handleChatError = (data: any) => {
      console.error("❌ Error creating chat via socket:", data.message);
      setLoading(false);
    ...
    return (
      ...
      >
        <View style={styles.avatarContainer}>
          ...
          {otherParticipant.userId.vehicleType && (
            <View style={[styles.vehicleBadge, { backgroundColor: getVehicleColor(otherParticipant.userId.vehicleType) }]}>
              <Ionicons
                name={getVehicleIcon(otherParticipant.userId.vehicleType)}
                size={10}
                color="#fff"
              />
            </View>
    ...
    return (
      ...
      >
        <View style={styles.avatarContainer}>
          ...
          {item.vehicleType && (
            <View style={[styles.vehicleBadge, { backgroundColor: getVehicleColor(item.vehicleType) }]}>
              <Ionicons
        ...
        <View style={styles.chatInfo}>
          ...
          <View style={styles.messageRow}>
            <CustomText fontSize={14} style={styles.lastMessage}>
              {item.role === "rider" ? `${item.vehicleType || "Rider"}` : "Passenger"} • Tap to start chat
            </CustomText>
          </View>
        </View>
      </TouchableOpacity>
